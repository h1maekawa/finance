...
const getCategoryStyle = (tx: any) => {
  const note = tx.note || ''
  if (note.includes('ライフ') || note.includes('スーパー') || note.includes('セブン')) return categoryInfo['食費']
...
<template>
  <div class="space-y-8 pb-32 animate-in fade-in duration-700">
    <!-- Welcome Section -->
    <section class="flex justify-between items-end px-1">
      <div class="space-y-1">
        <p class="text-premium-label transition-all">{{ userName }}さん、おはようございます</p>
        <h1 class="text-premium-headline text-3xl">家計の概況</h1>
      </div>
      <div v-if="lastSync" class="flex items-center gap-1.5 px-3 py-1.5 bg-surface-container-low rounded-full text-[11px] font-bold text-on-surface-variant border border-outline-variant/10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="card-premium relative overflow-hidden group md:col-span-2">
      ...
      <div class="card-premium flex flex-col space-y-6 relative overflow-hidden">
        ...
        <div class="flex justify-between items-center relative z-10">
          <h3 class="text-premium-headline text-lg text-secondary">総資産額</h3>
      ...
      <!-- Sync Info Card -->
      <div class="card-premium flex flex-col space-y-6 relative overflow-hidden">
        <div class="absolute -right-8 -bottom-8 w-32 h-32 bg-primary/5 rounded-full blur-3xl"></div>
        <div class="flex justify-between items-center relative z-10">
          <h3 class="text-premium-headline text-lg text-primary">自動取込</h3>
          <router-link to="/gmail-import" class="text-primary font-bold text-sm hov-underline">設定</router-link>
        </div>
        <div class="flex flex-col items-center justify-center py-4 gap-2 relative z-10">
          <span class="material-symbols-outlined text-primary text-4xl mb-2">auto_awesome</span>
          <p class="text-sm font-medium text-on-surface-variant">
            {{ lastSync ? `${lastSync} に同期済み` : '同期設定が必要です' }}
          </p>
          <button @click="fetchImportLogs" class="mt-2 text-[12px] font-bold text-primary px-4 py-1.5 bg-primary/5 rounded-full hover:bg-primary/10 transition-colors">
            状態を更新
          </button>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex items-center gap-4">
            <div :class="['w-12 h-12 flex items-center justify-center rounded-2xl transition-transform duration-500 hover:rotate-12', getCategoryStyle(tx).color]">
              <span class="material-symbols-outlined text-on-surface/70" style="font-variation-settings: 'FILL' 1;">
                {{ getCategoryStyle(tx).icon }}
              </span>
            </div>
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <p class="font-bold text-on-surface truncate pr-2">{{ tx.note || '未設定' }}</p>
                <span v-if="tx.import_source === 'gmail'" class="material-symbols-outlined text-[14px] text-primary" title="Gmailから自動取込">auto_awesome</span>
              </div>
